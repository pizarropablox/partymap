...
export class HomeMapComponent implements OnInit, OnDestroy {
  ...
  loadEventLocations() {
    ...
    this.http.get<any[]>(ApiEndpoints.EVENTO.ALL).subscribe({
      next: (eventos) => {
        if (eventos.length === 0) {
          this.markers = [];
          this.cdr.detectChanges();
  ...
  async verificarReservaExistente(): Promise<{ existe: boolean; reserva?: any }> {
    ...
    try {
      return new Promise((resolve) => {
        ...
        this.http.get<any>(ApiEndpoints.USUARIO.CURRENT, {
          ...
          next: (usuario) => {
            ...
            this.http.get<any[]>(ApiEndpoints.buildUrl(`/reserva/usuario/${usuarioId}`), {
              headers: { 'Authorization': `Bearer ${token}` }
            }).subscribe({
              next: (reservas) => {
